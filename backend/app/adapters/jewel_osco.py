"""Jewel-Osco adapter (Albertsons banner).

Data acquisition path
---------------------
Jewel-Osco is an Albertsons Companies banner.  Weekly ads run
Wednesday→Tuesday.

1. **Primary — Flipp API** (backflipp.wishabi.com).  Albertsons banners are
   on Flipp, keyed by **postal code**.  The :class:`FlippMixin` handles the
   two-step fetch.  We filter flyers by ``merchant_name`` matching
   "jewel-osco" (or "jewel osco").

2. **Fallback — browserless HTML scrape** of ``https://www.jewelosco.com/weeklyad.html``.

ToS note (spec §5.2): Albertsons' ToS restricts automated access.  Use the
official Albertsons Developer / partner feed where available.
"""

from __future__ import annotations

import logging
import re
from datetime import date, timedelta

from .base import AdMetadata, OfferData, StoreAdapter
from .flipp_mixin import FlippMixin

logger = logging.getLogger(__name__)

JEWEL_HTML = "https://www.jewelosco.com/weeklyad.html"


class JewelOscoAdapter(FlippMixin, StoreAdapter):
    STORE_KEY = "jewel_osco"
    STORE_NAME = "Jewel-Osco"
    DEFAULT_RATE_LIMIT = 2.0
    FLIPP_MERCHANT_PATTERN = r"jewel[-\s]*osco"

    def __init__(self, store_id: str = "jewel_osco", zip_or_store_id: str = "") -> None:
        super().__init__(store_id, zip_or_store_id or "default")

    async def fetch_current_ad(self) -> tuple[list[OfferData], AdMetadata]:
        metadata = AdMetadata(
            period_start=_wednesday(),
            period_end=_wednesday() + timedelta(days=6),
            store_location=self.zip_or_store_id,
        )

        # Try Flipp first (thread ZIP code)
        try:
            offers, meta = await self.fetch_flipp(self.zip_or_store_id)
            if offers:
                logger.info("jewel_osco: %d offers via flipp", len(offers))
                return offers, meta
        except Exception as exc:  # pragma: no cover
            logger.error("jewel_osco: flipp path failed: %s", exc)

        # Fallback: HTML scrape via browserless
        try:
            offers, meta = await self._fetch_via_html()
            if offers:
                logger.info("jewel_osco: %d offers via html", len(offers))
                return offers, meta
        except Exception as exc:  # pragma: no cover
            logger.error("jewel_osco: html path failed: %s", exc)

        logger.warning("jewel_osco: no offers retrieved")
        return [], metadata

    # ----------------------------------------------------------- HTML

    async def _fetch_via_html(self) -> tuple[list[OfferData], AdMetadata]:
        if not await self.check_robots_txt(JEWEL_HTML):
            logger.info("jewel_osco: robots.txt disallows %s", JEWEL_HTML)
            return [], AdMetadata(store_location=self.zip_or_store_id)

        from .base import BrowserClient

        html: str | None = None
        try:
            bc = BrowserClient()
            html = await bc.render_page(JEWEL_HTML)
        except Exception as exc:
            logger.debug("jewel_osco: browserless unavailable: %s", exc)

        if not html:
            resp = await self.fetch_with_retry(JEWEL_HTML)
            if resp is None:
                return [], AdMetadata(store_location=self.zip_or_store_id)
            html = resp.text

        self.save_raw_payload({"html": html[:50000]}, suffix="_html")
        offers: list[OfferData] = []
        for m in re.finditer(r'>([^<]{3,80}?)\s+\$?(\d+\.\d{2})[^<]*<', html):
            name = m.group(1).strip()
            price = self.dollars_to_cents(m.group(2))
            if price and name:
                offers.append(
                    OfferData(
                        raw_text=f"{name} ${m.group(2)}",
                        product_name=name,
                        price=price,
                        deal_type="sale",
                    )
                )
        meta = AdMetadata(
            period_start=_wednesday(),
            period_end=_wednesday() + timedelta(days=6),
            store_location=self.zip_or_store_id,
        )
        return offers, meta


# ------------------------------------------------------------------ helpers


def _wednesday() -> date:
    today = date.today()
    return today - timedelta(days=(today.weekday() - 2) % 7)
