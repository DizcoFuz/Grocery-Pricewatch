...
- **react-router-dom RSC CSRF advisory (GHSA — 2 high):** App runs `react-router-dom@7.18.2` (latest). The remaining audit advisories affect React Router `>=7.12.0 <8.3.0` and apply only to **framework mode** (RSC/SSR actions). This app is a client-side SPA (library mode), so it is not exploitable. Fix lands in v8.3.0 (unreleased).
